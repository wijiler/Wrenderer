...
spriteInstance createNewSpriteInstance(Sprite *sprite, renderer_t renderer, WREScene2D *scene)
{
    ...
    {
        scene->spritePipeline.spriteInstances = (spriteInstance *)realloc(scene->spritePipeline.spriteInstances, sizeof(spriteInstance) * (spriteInstanceCount + spriteIncrementAmount));
        scene->spritePipeline.spriteInstanceData = (transform2D *)realloc(scene->spritePipeline.spriteInstanceData, sizeof(transform2D) * (scene->spritePipeline.spriteInstanceCount + spriteIncrementAmount));
        scene->spritePipeline.textureIDs = (Material2D *)realloc(scene->spritePipeline.textureIDs, sizeof(Material2D) * (scene->spritePipeline.spriteInstanceCount + spriteIncrementAmount));

        if (spriteInstanceCount == 0)
        {
            BufferCreateInfo bci = {
                (int)(sizeof(transform2D) * (spriteInstanceCount + spriteIncrementAmount)),
                (BufferUsage)(BUFFER_USAGE_STORAGE_BUFFER | BUFFER_USAGE_TRANSFER_SRC | BUFFER_USAGE_TRANSFER_DST),
                CPU_ONLY,
            };
        ...
        {
            BufferCreateInfo bci = {
                (int)(sizeof(transform2D) * (spriteInstanceCount + spriteIncrementAmount - 1)),
                (BufferUsage)(BUFFER_USAGE_STORAGE_BUFFER | BUFFER_USAGE_TRANSFER_SRC | BUFFER_USAGE_TRANSFER_DST),
                CPU_ONLY,
            };
            {
...
{
    scene->lights = (pointLight2D *)realloc(scene->lights, sizeof(pointLight2D) * (lightCount + 1));
    if (lightCount == 0)
    {
        BufferCreateInfo bci = {
            (int)(sizeof(pointLight2D) * (lightCount + spriteIncrementAmount)),
            (BufferUsage)(BUFFER_USAGE_STORAGE_BUFFER | BUFFER_USAGE_TRANSFER_SRC | BUFFER_USAGE_TRANSFER_DST),
            CPU_ONLY,
        };
